	...
	<div class="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-100">
		<div class="flex items-center justify-between">
			...
			<div class="flex items-center gap-2">
				<button id="cal-prev-dosen" class="h-8 w-8 rounded-full bg-slate-100 grid place-items-center">‹</button>
				<div id="cal-month-dosen" class="text-sm font-semibold text-slate-700"></div>
				<button id="cal-next-dosen" class="h-8 w-8 rounded-full bg-slate-100 grid place-items-center">›</button>
			</div>
		</div>
		<div class="mt-3 grid grid-cols-7 text-center text-xs font-semibold text-slate-500">
			<span>Su</span><span>Mo</span><span>Tu</span><span>We</span><span>Th</span><span>Fr</span><span>Sa</span>
		</div>
		<div id="cal-grid-dosen" class="mt-2 grid grid-cols-7 gap-1 text-center text-sm"></div>
	</div>
</div>
...
<script>
(function(){
	const monthEl = document.getElementById('cal-month-dosen');
	const gridEl = document.getElementById('cal-grid-dosen');
	const prevBtn = document.getElementById('cal-prev-dosen');
	const nextBtn = document.getElementById('cal-next-dosen');

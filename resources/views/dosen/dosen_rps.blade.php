				...
				<div class="border border-slate-200 rounded-xl px-6 py-4 hover:border-emerald-400 hover:shadow-md transition-all bg-gradient-to-r from-white to-slate-50">
					<div class="flex items-center justify-between">
						...
						<div class="flex items-center gap-4 ml-6">
							...
							<!-- Input - always show -->
							<a :href="c.has_rps && c.rps_id ? `{{ route('dosen.input_rps', ['code'=>$code]) }}?rps_id=${c.rps_id}` : `{{ route('dosen.input_rps', ['code'=>$code]) }}?kode=${c.kode || ''}&semester=${c.semester || ''}`" class="inline-flex items-center gap-2 text-sm text-blue-600 hover:text-blue-700 font-medium transition-colors">
								<span class="i-heroicons-pencil text-lg"></span>
								<span x-text="c.has_rps ? 'Edit' : 'Input'"></span>
							</a>

							<!-- Komentar - only show if RPS exists -->
